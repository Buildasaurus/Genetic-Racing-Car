class NeuralNetwork {
  ...
  //All biases
  float[] biases = {0,0,0,0};//new float[3];
  ...
  float getOutput(float x1, float x2, float x3) {
    ...
    //layer2
    return 1/(1+exp(-o11*weights[6] - o12*weights[7] - biases[2]))-0.5;
  }
  
  float getPasta(float x1, float x2, float x3) 
  {
    //layer1
    float o11 = weights[0]*x1+ weights[1]*x2 + weights[2]*x3 + biases[0];
    float o12 = weights[3]*x1+ weights[4]*x2 + weights[5]*x3 + biases[1];
    //layer2
    return 1/(1+exp(-o11*weights[8] - o12*weights[9] - biases[3]));
  }
}
